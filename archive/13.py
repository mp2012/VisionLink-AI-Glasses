...
字体：系统黑体 simhei.ttf
功能：按键提示音 + 新语音自动终止旧语音，仅播放最新语音
依赖：pip install pillow opencv-python ollama SpeechRecognition
快捷键：
  1-5    切换功能
  空格    执行当前功能
  L       切换中英双语
...
PROMPT_LIB = {
    ...
    "en": [
        ...
        "Read all printed text in the picture briefly in English.",
...
def main():
    ...
    while True:
        ...
        if agent_auto_enable and now_t - last_agent_run_time >= AGENT_SCAN_INTERVAL and current_state == STATE_IDLE:
            ...
            tasks = agent_scene_detect(frame)
            if tasks:
                threading.Thread(target=agent_auto_execute, args=(frame.copy(),), daemon=True).start()
