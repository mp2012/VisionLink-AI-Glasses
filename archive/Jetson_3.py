"""
VisionLink Jetson 纯手动无头兼容版
特性：自动识别有无显示器，无头环境关闭所有OpenCV图形窗口，大幅降低内存占用
仅空格键手动AI识别，无自动扫描，无按键蜂鸣音
"""
import os
import sys
...
try:
    ...
    logger.info("✅ 中文字体加载成功 wqy-microhei（仅文本存储使用）")
except Exception as e:
    logger.error(f"❌ 字体缺失，请执行 sudo apt install fonts-wqy-microhei | {e}")
    sys.exit(1)

# ====================== 全局参数（轻量化适配8G Jetson） ======================
MODEL_NAME = "gemma4:e2b-q4_K_S"
FRAME_WIDTH = 640
FRAME_HEIGHT = 480
AI_IMAGE_SIZE = 288
JPEG_QUALITY = 70
...
def tts_speak(text: str):
    ...
    espeak_cmd = (
        f"espeak -v zh+f2 -s 140 --wav={wav_save_path} '{clean_text}' >/dev/null 2>&1;"
        f"aplay -q {wav_save_path} >/dev/null 2>&1"
    )
    try:
...
def run_infer_task(frame, mode_idx):
    ...
    def infer_work():
        ...
        try:
            ...
            logger.info(f"✅ 推理结果：{result_text}")
...
def main():
    ...
    if HAS_DISPLAY:
        logger.info("检测到显示器，启用预览窗口")
    else:
        logger.info("【无头模式运行】无显示器，关闭OpenCV图形窗口，节省内存")
    ...
    logger.info("运行模式：仅空格键手动AI识别，无自动后台扫描")
    logger.info("=" * 60)
    ...
    try:
        ...
        stop_all_tts()
        logger.info("摄像头、语音进程全部释放，程序正常结束")
