...
字体：系统黑体 simhei.ttf
功能：按键提示音 + 新语音自动终止旧语音（基于PowerShell，无硬件死锁）
修复：完美分离UI防止视觉幻觉 + 自动本地保存被识别的干净图片
快捷键：
  1-5    切换功能
  空格    执行当前功能
  L       切换中英双语
...
def func_obstacle(frame):
    ...
    save_clean_frame(frame, 1)  # 保存无UI的干净图片
...
def func_ocr(frame):
    ...
    save_clean_frame(frame, 2)  # 保存无UI的干净图片
    img_b64 = image_to_base64(frame)
...
def func_face(frame):
    ...
    save_clean_frame(frame, 3)  # 保存无UI的干净图片
    img_b64 = image_to_base64(frame)
...
def func_scene(frame):
    ...
    save_clean_frame(frame, 4)  # 保存无UI的干净图片
    img_b64 = image_to_base64(frame)
...
def func_chat(frame):
    global context_history, current_state
    logger.info("执行功能5：语音交互")
    save_clean_frame(frame, 5)  # 保存无UI的干净图片
...
def main():
    ...
    while True:
        ...
        # 🛠️ 【核心修复核心点 1】：在渲染UI前，先深度拷贝一份绝对纯净的画面
        ...
        if key == 32:
            ...
            # 🛠️ 【核心修复核心点 2】：空格触发时，只传没有被画上UI的纯净图 raw_frame 给大模型
